...
RSpec.describe Resolver, "with triangular dependencies" do
  ...
  context "when the dependency between 'topics' and 'oil_and_gas' is removed" do
    ...
    describe "#dependees(node)" do
      it "returns all nodes which are depended on by the given node" do
        expect(resolver.dependees(fields_and_wells)).to eq([topics].to_set)
        expect(resolver.dependees(oil_and_gas)).to eq([fields_and_wells, topics].to_set)
    ...
    describe "#dependents(node)" do
      it "returns all nodes which depend on the given node" do
        expect(resolver.dependents(fields_and_wells)).to eq([oil_and_gas].to_set)
  ...
  context "when the dependency between 'oil_and_gas' and 'fields_and_wells' is removed" do
    before do
      resolver.remove_dependency(dependent: oil_and_gas, dependee: fields_and_wells)
    end
  ...
  context "when the dependency between 'fields_and_wells' and 'topics' is removed" do
    ...
    describe "#dependees(node)" do
      it "returns all nodes which are depended on by the given node" do
        ...
        expect(resolver.dependees(oil_and_gas)).to eq([fields_and_wells].to_set)
        expect(resolver.dependees(topics)).to eq([oil_and_gas, fields_and_wells].to_set)
      end
    end
    ...
    describe "#dependents(node)" do
      it "returns all nodes which depend on the given node" do
        expect(resolver.dependents(fields_and_wells)).to eq([oil_and_gas, topics].to_set)
